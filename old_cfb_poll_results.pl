...
if ($help) {print $usage}
	...
	my %Poll_names = ("http://collegefootball.ap.org/poll/$year/$week" => 'AP',
...
sub get_team_votes {
	...
	if ($url =~ /ap\.org/) {
		...
		for my $line (split qr /\R/, $content) {
			...
			for my $chunk (split '><', $line) {
				...
				if ($chunk =~ /\"\/teams/) {
					$chunk =~ s/Mississippi/Ole Miss/g;
					$chunk =~ s/&amp;/&/;
	...
	# 		if ($_ =~ /$year Weekly FEI Ratings/) {

	# 			while (<FEI>) {

	# 				if ($_ =~ /align=\"center\">\d+</) {
	# 					my ($team) = <FEI> =~ /align=\"left\">(.+)</;
	# 					# print "$team\t";
