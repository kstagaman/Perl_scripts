#!/usr/bin/perl
# order_bt2_ig_matched.pl
use strict; use warnings;
use Getopt::Long;

my $usage = "\n\torder_bt2_ig_matched.pl [options -h] -d <PATH to fasta files> -i <CSV stats file>\n\n";
...
if ($help) {
	...
	while (<CSV>) {
		...
		if  ($read1 =~ /igh[mz]/ and $read2 =~ /igh[mz]/) {
			print OUT "$sample,$seq_id,cc,$read1,$read2\n";
			$bad_match++;
		}
		elsif ($read1 =~ /ighj/ and $read2 =~ /ighj/) {
			print OUT "$sample,$seq_id,jj,$read1,$read2\n";
			$bad_match++;
		}
		elsif ($read1 =~ /ighj/ and $read2 =~ /igh[mz]/) {
			print OUT "$sample,$seq_id,jc,$read1,$read2\n";
			$bad_match++;
		}
		elsif ($read1 =~ /igh[mz]/ and $read2 =~ /ighj/) {
			print OUT "$sample,$seq_id,jc,$read1,$read2\n";
			$bad_match++;
		}
		elsif ($read1 =~ /ighv/ and $read2 =~ /ighv/) {
			print OUT "$sample,$seq_id,vv,$read1,$read2\n";
			$bad_match++;
		}
